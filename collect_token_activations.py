...
class TokenSpecificActivationCollector:
    def __init__(self, model_path, layer_names, target_layer_name, device_id,
                 ...
                 padding_side, truncation_side, model_max_length, target_tokens_file,
                 pos_output_path, neg_output_path, generation_max_length, temperature_val,
                 top_k_val, top_p_val, repetition_penalty_val, do_sample_flag, num_beams,
        ...
        if not target_tokens_file:
            raise ValueError("target_tokens_file is required")
        ...
        self.target_tokens_file = target_tokens_file
        ...
        with open(target_tokens_file, 'r') as f:
            self.target_tokens = [line.strip() for line in f.readlines()]
    ...
    def load_model_and_tokenizer(self):
        ...
        for token in self.target_tokens:
            ...
            if len(token_id) == 1:
                self.target_token_ids.append(token_id[0])
            else:
                self.target_token_ids.extend(token_id)
            ...
            for i, activation_tensor in enumerate(self.activations):
                ...
                for batch_idx in range(batch_size):
                    for seq_idx in range(seq_len):
                        if seq_idx < len(generated_tokens):
                            current_token_id = generated_tokens[seq_idx].item()
                            activation_vector = activation_tensor[batch_idx, seq_idx, :]
                            
                            if current_token_id in self.target_token_ids:
...
def main():
    parser = argparse.ArgumentParser(description='Collect token-specific activations for local control')
    ...
    parser.add_argument('--target_tokens_file', required=True, help='Target tokens file')
    ...
    collector = TokenSpecificActivationCollector(
        ...
        args.padding_side, args.truncation_side, args.model_max_length, args.target_tokens_file,
        args.pos_output_path, args.neg_output_path, args.generation_max_length, args.temperature_val,
        args.top_k_val, args.top_p_val, args.repetition_penalty_val, args.do_sample_flag == "True", args.num_beams,
